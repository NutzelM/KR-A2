...
def normalise(cpt_to_be_normalised, evidence):
    ...
    for var in evidence.index.values:
        ...
        if len(cpt) > 1: 
            keys_to_multiply = find_keys_to_multiply(all_cpts, var)
...
def multiply_cpts(all_cpts, keys_to_multiply):
    ...
    if len(new_keys_to_multiply) > 1:
        cpt = multiply_cpts(all_cpts, new_keys_to_multiply)
    return cpt
...
def find_keys_to_multiply(all_cpts, var):
    """ 
...
#############################
def prune_edges(all_cpts, evidence):
    """ 
    ...
    for evi in E:
        ...
        if len(list(neighbours)) > 0:
            for e in neighbours:
                ...
                all_cpts[e] = sum_out(cpt_reduced, evi)
    return all_cpts

def prune_nodes(all_cpts, evidence, Q):
    """ 
    ...
    return prune_nodes(evidence, Q)

def prune_network(all_cpts, evidence,  Q):           
    """ 
    node and edge prunes a network
    returns : all_cpts
    """
    all_cpts_new = prune_nodes(all_cpts, evidence, Q)
    all_cpts_new = prune_edges(all_cpts, evidence)
    ...
    for key in all_cpts_new:
        if not all_cpts_new[key].equals(all_cpts[key]):
            return prune_network(all_cpts_new, evidence, Q)
    return all_cpts_new

#############################
# Ordering methods
#############################
def order(variables, ordering):
    """
    ...
    if ordering == 'min_degree': 
        return  min_degree_order(variables)
    if ordering == 'random':
        return variables
...
def prior_marginal(Q, ordering): 
    ...
    return posteriour_marginal(Q, pd.Series({}), ordering)

def posteriour_marginal(Q, evidence, ordering): 
    """ 
    ...
    #  multiply remaining
    if len(all_cpts) > 1:
        final_cpt = multiply_cpts(all_cpts, list(all_cpts.keys()))
    ...
    for c in list(final_cpt.columns.values)[:-1]:
        if c not in Q:
            final_cpt = sum_out(final_cpt, c)
    return final_cpt

def MPE(evidence, ordering):
    """ 
    ...
    """
    all_cpts = BN.get_all_cpts()
    all_cpts = prune_edges(all_cpts, evidence)
...
def MAP(M, evidence, ordering):
    ...
    """
    all_cpts = BN.get_all_cpts()
    all_cpts = prune_network(all_cpts, evidence, [])
    Q = BN.get_all_variables()
    pi = order(Q, ordering)
    pi_ordered_M = []
    for var in pi:
