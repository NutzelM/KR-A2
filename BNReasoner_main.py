...
class BNReasoner:
    ...
    def prune_nodes(self, all_cpts, evidence, Q):
        ...
        for key in all_cpts:
            ...
            if key not in E and key not in Q and self.bn.get_children(key) == None:
                self.bn.del_var(key)
                all_cpts.pop(key)
                pruned_bool = True
...
if __name__ == '__main__':
    ...
    #         old_MAP_dataframe = MAP_dataframe
    #     MPE_2d_list = []
    #     MAP_2d_list = []
    ...
    #             start = time.time()
    #             result = network.MAP(M, evidence, order)
    #             end = time.time()
    #             print(f"appending {end - start} to columns {order}")
    #             MAP_1d_list.append(end-start)
    #         MPE_2d_list.append(MPE_1d_list)
    #         MAP_2d_list.append(MAP_1d_list)
    #     print(MPE_2d_list)
    #     MPE_dataframe = pd.DataFrame(MPE_2d_list, columns = ['min degree heuristic', 'min fill heurisic', 'random (no heuristic)'])
    #     MAP_dataframe = pd.DataFrame(MPE_2d_list, columns = ['min degree heuristic', 'min fill heurisic', 'random (no heuristic)'])
    #     if j > 0:
    #         MPE_dataframe = (MPE_dataframe + old_MPE_dataframe)/2
    #     print(MPE_dataframe)
    # MPE_dataframe.to_csv('MPEdataframe.txt', index=False, sep=' ')
    # MAP_dataframe.to_csv('MAPdataframe.txt', index=False, sep=' ')
